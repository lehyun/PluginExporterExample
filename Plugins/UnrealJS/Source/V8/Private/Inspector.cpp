...
public:
	...
	int inspector_server( lws *Wsi, lws_callback_reasons Reason,	void *User, void *In, size_t Len )
	{
		...
		case LWS_CALLBACK_HTTP:
		{
			...
			{
				auto Body = FString::Printf(TEXT("[{%s}]"), *FString::Join(res2, TEXT(",")));
				FTCHARToUTF8 BodyUtf8(*Body);

				auto Header = FString::Printf(TEXT("HTTP/1.0 200 OK\r\nContent-Type: application/json; charset=UTF-8\r\nContent-Length: %d\r\nConnection: closed\r\nCache-Control: no-cache\r\n\r\n"), BodyUtf8.Length());
				FTCHARToUTF8 HeaderUtf8(*Header);

				lws_write_http(Wsi, HeaderUtf8.Get(), HeaderUtf8.Length());
				lws_write_http(Wsi, BodyUtf8.Get(), BodyUtf8.Length());
			}
		}
		break;
